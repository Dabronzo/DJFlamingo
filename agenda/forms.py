...
class GigCreationForm(ModelForm):
    ...
    date = forms.DateField(widget=DateInput(attrs={'class': 'form-control', 'placeholder': ''}))
    is_payed = forms.BooleanField(required=False)

    class Meta:
        """Meta class for widgets"""
        model = Gig
        fields = ('name', 'date', 'time_start', 'time_duration', 'dj', 'venue', 'fees', 'is_payed', 'status', 'notes')

        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Event Name'}),
            'time_start': forms.TimeInput(attrs={'type': 'time'}),
            'time_duration': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Event Name'}),
...
class VenueCreationForm(ModelForm):
    ...
    class Meta:
        ...
        model = Venue
        fields = ('name', 'address', 'city', 'website', 'contact', 'additional_info', )
